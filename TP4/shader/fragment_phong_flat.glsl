...
/* -- Données en entrée -- */
flat in vec4 posInWorldSpace;         // point p (position) dans le repère monde
flat in vec4 normalInWorldSpace;      // normales dans le repère monde
flat in vec4 cameraPosInWorldSpace;   // caméra dans le repère monde
...
void main() {
  ...
  L_a = vec3(0.1176, 0.5412, 0.3412);
  L_d = vec3(0.1176, 0.5412, 0.3412);
  L_s = vec3(0.1176, 0.5412, 0.3412);
